...
class DatabaseManager:
    ...
    def get_connection(self):
        ...
        while retry_count < max_retries:
            try:
                ...
                retry_count += 1
                error_msg = "Database connection failed (attempt %d/%d): %s"
                
                if retry_count < max_retries:
                    logging.warning(error_msg + ". Retrying...", retry_count, max_retries, e)
                    continue
                else:
                    logging.error(error_msg, retry_count, max_retries, e)
    ...
    def test_connection(self) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                return result[0] == 1        except DatabaseError:
            raise
        except Exception as e:
    ...
    def save_conversation_history(self, user_id: str, conversation_data: str) -> bool:
        ...
        try:
            ...
            logging.debug(f"Conversation history saved for user {user_id}")
            return True
            
        except Exception as e:
            logging.error(f"Failed to save conversation history for user {user_id}: {e}")
            return False
    ...
    def cleanup_old_conversations(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            logging.info(f"Cleaned up {rows_affected} old conversation records")
